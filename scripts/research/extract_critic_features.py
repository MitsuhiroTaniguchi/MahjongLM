"""Features for the (B) short-horizon advantage critic + advantage-rank conditioning.

Per viewer DISCARD decision, from the FROZEN imperfect base model:
  * hidden_state  : hidden at pos-1 (BEFORE the discard) -> state s, for Q(s,a)
  * base_logp     : log pi0 over the 74 canonical discards (the in-support actions)
  * aplus         : canonical index of the chosen discard
  * dealin        : 1 if this discard was immediately RON'd (short-horizon, detectable)
  * round_bucket  : the round score-delta bucket 0..4 (long-horizon, diluted)
  * game          : game index (train/test split)

No rollout: dealin / round outcome are read from the realized trajectory; the
critic Q(s,a) is a feedforward function of (state, candidate action).
"""
...
def main():
    ...
    ap.add_argument("--out", default="outputs/research/critic_features.npz")
    args = ap.parse_args()
    ...
    np.savez_compressed(args.out,
                        hidden=np.asarray(H, dtype=np.float16), base_logp=np.asarray(LP, dtype=np.float16),
