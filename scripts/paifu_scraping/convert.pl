...
sub player {
    ...
    my @player = map { "$name[$_] ($dan[$_] R$rate[$_])" } (0 .. $player_count - 1);
    return \@player;
}
...
sub qipai {
    ...
    my @ten_all = map { $_ * 100 } split(',', $attr{ten});
    my @ten = @ten_all[0 .. $player_count - 1];
    my @hai = map { pai(split(',', $attr{"hai$_"})) } (0 .. $player_count - 1);
...
sub hule {
    ...
    my @sc_all  = map { $_ * 100 } (split(',', $attr{sc}))[1,3,5,7];
    my @sc      = @sc_all[0 .. $player_count - 1];
...
sub pingju {
    ...
    my @hai = map {
                (defined $attr{"hai$_"})
                    && join(',', pai(split(',', $attr{"hai$_"})), @{$fulou[$_]}, @{$nuki[$_]})
             } (0 .. $player_count - 1);
...
for (join('', <>) =~ /<.*?>/g) {
    ...
    if ($elem eq 'mjloggm') {
        ...
        my $l = (ord($1) - ord('T') + $player_count - $oya) % $player_count;
        my $p = pai($2);
        if ($gang) {
        ...
        my $l = (ord($1) - ord('D') + $player_count - $oya) % $player_count;
        my $p = pai($2);
        $p .= '_'   if ($2 eq $zimo);
        $p .= '*'   if ($lizhi);
        ...
        my $l = ($attr{who} + $player_count - $oya) % $player_count;
        my $m = mianzi($attr{m});
        if ($m =~ 'b') {
            push(@$log, { penuki => { l => $l, p => 'z4' } });
            push(@{$nuki[$attr{who}]}, $m);
        }
        elsif ($m =~ /^[mpsz]\d{4}$/ || $m =~ /^[mpsz]\d{3}[\-\+\=]\d$/) {
    ...
    if ($attr{owari}) {
        ...
        $paipu->{defen} = [ map { $_ * 100 } @owari[map { $_ * 2 } (0 .. $player_count - 1)] ];
        $paipu->{point} = [ @owari[map { $_ * 2 + 1 } (0 .. $player_count - 1)] ];

        my @rank  = ((0) x $player_count);
        for (my $i = 0; $i < $player_count; $i++) {
            for (my $j = 0; $j < $player_count; $j++) {
                if ($j <= $i) {
                    $rank[($paipu->{qijia} + $i) % $player_count]++
                        if ($paipu->{defen}[($paipu->{qijia} + $j) % $player_count]
                                >= $paipu->{defen}[($paipu->{qijia} + $i) % $player_count]);
                }
                else {
                    $rank[($paipu->{qijia} + $i) % $player_count]++
                        if ($paipu->{defen}[($paipu->{qijia} + $j) % $player_count]
