param(
    [ValidateSet("baseline", "xsa", "gated")]
    [string]$Variant = "baseline",
    [string]$ProjectName = "mahjongLM_qwen3_100m_compare",
    [string]$RunPrefix = "qwen100",
    [string]$DatasetDir = "data/processed/2011",
...
$variantArgs = switch ($Variant) {
    "baseline" { @() }
    "xsa" { @("--use-exclusive-self-attention") }
...
$allArgs = @($baseArgs + $variantArgs + $ExtraArgs)
